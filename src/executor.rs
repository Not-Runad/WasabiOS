...
/// For wrapper of Future where is defined
pub struct Task<T> {
...
#[derive(Default)]
pub struct Yield {
    polled: AtomicBool, // is already polled
}
impl Future for Yield {
    type Output = ();
    fn poll(self: Pin<&mut Self>, _cx: &mut Context<'_>) -> Poll<Self::Output> {
        // for the first time poll, return Pending.
        // Afterwards, return Ready.
...
/// No operation Waker from no operation RawWaker
...
pub fn block_on<T>(future: impl Future<Output = Result<T>> + 'static) -> Result<T> {
    ...
    // block execution until return Future
    loop {
        let waker = no_op_waker();
        let mut context = Context::from_waker(&waker);
        match task.poll(&mut context) {
