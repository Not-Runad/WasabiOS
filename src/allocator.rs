...
impl Header {
    ...
    fn provide(&mut self, size: usize, align: usize) -> Option<*mut u8> {
        ...
        if self.is_allocated() || !self.can_provide(size, align) {
            ...
            let mut header_for_allcated =
                unsafe { Self::new_from_addr(allocated_addr - HEADER_SIZE) };
            header_for_allcated.is_allocated = true;
            header_for_allcated.size = size + HEADER_SIZE;
            size_used += header_for_allcated.size;
            header_for_allcated.next_header = self.next_header.take();

            // If need padding
            if header_for_allcated.end_addr() != self.end_addr() {
                // Make a Header for padding
                let mut header_for_padding =
                    unsafe { Self::new_from_addr(header_for_allcated.end_addr()) };
                header_for_padding.is_allocated = false;
                header_for_padding.size = self.end_addr() - header_for_allcated.end_addr();
                size_used += header_for_padding.size;
                header_for_padding.next_header = header_for_allcated.next_header.take();
                header_for_allcated.next_header = Some(header_for_padding);
            }

            // Shrink this empty area
            assert!(self.size >= size_used + HEADER_SIZE);
            self.size -= size_used;
            self.next_header = Some(header_for_allcated);
...
impl FirstFitAllocator {
    ...
    fn add_free_from_descriptor(&self, desc: &EfiMemoryDescriptor) {
        ...
        // Make sure allocator does not include the address 0 as a free area.
        if start_addr == 0 {
            start_addr += 4096;
...
// FirstFitAllocator is NOT thread safe now.
// But now OS has ONLY one thread.
