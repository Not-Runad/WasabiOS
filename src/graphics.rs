...
pub fn draw_font_fg<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64, c: char) {
...
pub fn draw_str_fg<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64, s: &str) {
...
pub fn draw_test_pattern<T: Bitmap>(buf: &mut T) {
    ...
    draw_str_fg(buf, 0x00ff00, left, h * colors.len() as i64 + 16, "ABCDEF");
}
...
impl<T: Bitmap> fmt::Write for BitmapTextWriter<T> {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        for c in s.chars() {
            ...
            draw_font_fg(&mut self.buf, 0xffffff, self.cursor_x, self.cursor_y, c);
            self.cursor_x += 8;
        }
        Ok(())
    }
