...
// Limiting the size of TrbRing to be equal or less than 4096 to avoid crossing 64KiB boundaries.
// See Table 6-1 of xhci spec.
...
struct EventRingSegmentTableEntry {
    /// The physical address for segement base.
    ring_segment_base_address: u64,
    /// The number of segments.
    ring_semgent_size: u16,
    _rsvdz: [u16; 3],
}
...
impl EventRing {
    ...
    fn pop(&mut self) -> Result<Option<GenericTrbEntry>> {
        ...
        unsafe {
            let erdp = self.erdp.expect("erpd is no set");
...
impl OperationalRegisters {
    ...
    fn set_num_device_slots(&mut self, num_slots: usize) -> Result<()> {
        let c = self.config.read();
        let c = c & !0xff;
        let c = c | num_slots as u64;
        self.config.write(c);
        Ok(())
    }
...
impl PciXhciDriver {
    pub fn supports(vendor_device_id: VendorDeviceId) -> bool {
        ...
        VDI_LIST.contains(&vendor_device_id)
    }
    ...
    pub fn attach(pci: &Pci, bus_device_function: BusDeviceFunction) -> Result<()> {
        info!("xHCI found at: {bus_device_function:?}");
        pci.disable_interrupt(bus_device_function)?;
        pci.enable_bus_master(bus_device_function)?;
        let bar0 = pci.try_bar0_mem64(bus_device_function)?;
        bar0.disable_cache();
