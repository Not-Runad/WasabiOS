...
impl Hpet {
    pub fn new(registers: &'static mut HpetRegisters) -> Self {
        // The count-up period is recorded in the upper 32 bits of the capabilities_and_id register.
        // The increase is 1 in 1E-15 seconds i.e. 1 femto seconds.
        let fs_per_count = registers.capabilities_and_id >> 32;
        let num_of_timers = ((registers.capabilities_and_id >> 8) & 0x11111) as usize + 1;
        let freq = 1e15 as u64 / fs_per_count;
        let mut hpet = Self {
            registers,
            num_of_timers,
            freq,
        ...
        unsafe {
            // Disabled because interrupt occur is not used
            ...
            // set main_counter_value used by all counter to 0
...
pub fn global_timestamp() -> Duration {
    if let Some(hpet) = &*HPET.lock() {
        let ns = hpet.main_counter() as u128 * 1e9 as u128 / hpet.freq() as u128;
